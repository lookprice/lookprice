              ...
              <section className="max-w-7xl mx-auto px-4 md:px-8 py-16 md:py-32">
                <div className="flex flex-col md:flex-row items-end justify-between gap-6 mb-16">
                  ...
                  <div className="flex items-center gap-4 text-xss font-bold text-slate-400 tracking-wide">
                    ...
                    <span className="text-slate-900 font-semibold">{products.some(p => p.labels?.includes("Kampanya") || p.labels?.includes("Fırsat")) ? products.filter(p => p.labels?.includes("Kampanya") || p.labels?.includes("Fırsat")).length : Math.min(8, products.length)}</span>{" "}
                    {lang === "tr" ? "Parça" : "Items"}
                  </div>
                </div>

                <CampaignCarousel products={products} lang={lang} isLuxury={isLuxury} setSelectedProduct={setSelectedProduct} formatPrice={formatPrice} store={store} />
              </section>
            )}
