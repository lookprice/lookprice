...
export const BookstoreBadgePopover: React.FC<BookstoreBadgePopoverProps> = ({
  ...
  const badges = getSectorBadges(storeType);
  const isTr = lang === 'tr';

  return (
    <div 
      className="absolute left-0 top-full mt-1 w-56 bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-slate-200 dark:border-slate-800 p-2 z-50 animate-in fade-in zoom-in-95 duration-100 origin-top-left"
      onClick={(e) => e.stopPropagation()}
    >
      <div className="flex items-center justify-between pb-1.5 mb-1.5 border-b border-slate-100 dark:border-slate-800">
        <span className="text-[10px] font-black uppercase tracking-wider text-slate-800 dark:text-slate-100 flex items-center gap-1">
          <Sparkles className="w-3 h-3 text-indigo-600 dark:text-indigo-400" />
          <span>{isTr ? "Vitrin Izgara Rozetleri" : "Showcase Badges"}</span>
        </span>
        <button
          type="button"
          onClick={(e) => {
        ...
        {badges.map((b) => {
          ...
          return (
            <button
              ...
              className={`w-full px-2 py-1.5 rounded-lg text-[10px] font-bold flex items-center justify-between transition-all border cursor-pointer select-none text-left ${
                active 
            ...
            >
              <div className="flex items-center gap-1.5 truncate">
                <IconComp className={`w-3.5 h-3.5 shrink-0 ${active ? "text-current" : b.textClass}`} />
                <span className="truncate">{isTr ? b.labelTr : b.labelEn}</span>
              </div>
              {active ? (
                <Check className="w-3.5 h-3.5 shrink-0 ml-1" />
              ) : (
                <Plus className="w-3.5 h-3.5 shrink-0 opacity-40 ml-1" />
              )}
            </button>
          );
        })}
      </div>
      <p className="text-[9px] text-slate-400 mt-1.5 pt-1 border-t border-slate-100 dark:border-slate-800 leading-tight">
        {isTr ? "İşaretlenen ürün anında web sitesindeki vitrin ızgarasında gösterilir." : "Products appear instantly in the selected showcase row."}
      </p>
    </div>
  );
};
