...
export const PortfolioWebsiteGenerator = ({ storeId }: { storeId?: number }) => {
  ...
  useEffect(() => {
    if (storeId) {
      ...
      api.getBlogPosts(storeId).then((res) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error(error);
      alert(lang === 'tr' ? 'Kaydedilirken bir hata oluştu.' : 'An error occurred while saving.');
    }
  };
  ...
  const [sections, setSections] = useState<SectionConfig[]>([
    ...
    { id: 'news', label: lang === 'tr' ? 'Bölgesel Haberler & Gelişmeler' : 'Regional News & Updates', icon: Newspaper, enabled: true },
    { id: 'blog', label: lang === 'tr' ? 'Blog Yazıları' : 'Blog Posts', icon: Newspaper, enabled: true },
                    ...
                    <div className="space-y-3">
                       <div className="flex items-center justify-between">
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                            {lang === 'tr' ? 'Standart Mağaza Linki (Ücretsiz)' : 'Standard Store Link (Free)'}
                          </label>
                          <div className={`h-4 w-8 rounded-full flex items-center transition-colors p-0.5 cursor-pointer ${!useCustomDomain ? 'bg-indigo-500' : 'bg-slate-200'}`} onClick={() => setUseCustomDomain(false)}>
                    ...
                    <div className="space-y-3">
                       <div className="flex items-center justify-between">
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                            {lang === 'tr' ? 'Özel Domain (Cloudflare)' : 'Custom Domain (Cloudflare)'}
                            <span className="bg-amber-100 text-amber-600 text-[8px] px-1.5 py-0.5 rounded-md">PRO</span>
                          </label>
                          <div className={`h-4 w-8 rounded-full flex items-center transition-colors p-0.5 cursor-pointer ${useCustomDomain ? 'bg-indigo-500' : 'bg-slate-200'}`} onClick={() => setUseCustomDomain(true)}>
                             <div className={`h-3 w-3 bg-white rounded-full shadow-sm transition-transform ${useCustomDomain ? 'translate-x-4' : 'translate-x-0'}`}></div>
                          </div>
                       </div>
                       
                       <div className={`space-y-4 transition-all ${useCustomDomain ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}>
                         ...
                         <div className="space-y-2">
                            ...
                            <div className="p-3 bg-slate-900 rounded-xl font-mono text-[9px] text-emerald-400 space-y-1">
                               <p>CNAME {customDomain || 'domain.com'} → nodes.lookprice.net</p>
               ...
               <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                 ...
                 <div className="space-y-4">
                   <div className="p-4 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                      ...
                      <p className="text-emerald-700 text-[10px] mb-1">https://{currentUrl}</p>
                      <p className="text-slate-500 text-[10px] line-clamp-2">{lang === 'tr' ? 'Bölgenin en seçkin portföyü. Ekibimizle birlikte güvenli yatırım adımlarını keşfedin.' : 'The most exclusive portfolio in the region. Discover safe investment steps with our team.'}</p>
                   </div>
            ...
            <div className="bg-white p-8 rounded-3xl border border-slate-900 bg-slate-900 text-white space-y-6">
               ...
               <div className="text-center space-y-2">
                  ...
                  <p className="text-[10px] font-medium text-slate-400 leading-relaxed">
                    Tüm fazlar tamamlandı. Sitenizi yayınladığınızda <strong className="text-white">LookPrice Master Portföy</strong> gücüyle yayına girecektir. 
                    {useCustomDomain ? ' DNS yönlendirmeleri tamamlandıysa siteniz kısa süre içinde aktif olacaktır.' : ' Mağaza linkiniz hemen aktif olacaktır.'}
                  </p>
               </div>
               <a href={`https://${currentUrl}`} target="_blank" rel="noreferrer" className="w-full py-4 bg-indigo-600 rounded-2xl text-white text-[11px] font-black uppercase tracking-[0.2em] hover:bg-indigo-700 transition-all flex items-center justify-center gap-2">
           ...
           <div className="bg-slate-900 rounded-[3rem] border-[16px] border-slate-800 shadow-2xl overflow-hidden min-h-[850px] flex flex-col sticky top-6">
              ...
              <div className="bg-slate-800 p-5 flex items-center gap-4 border-b border-slate-700/50">
                 ...
                 <div className="bg-slate-900/60 flex-1 py-1.5 rounded-2xl flex items-center justify-center gap-2 group cursor-pointer border border-slate-700/50">
                    ...
                    <span className="text-[11px] font-bold text-slate-500 tracking-tight">{currentUrl}</span>
                 </div>
              </div>
                            ...
                            <div className="space-y-2">
                               ...
                               <div className="flex items-center gap-4">
                                  ...
                                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">
                                     {lang === 'tr' ? 'LookPrice Entegre Veri Akışı' : 'LookPrice Integrated Data Feed'}
                                  </p>
                               </div>
                            </div>
                            ...
                            <div className="space-y-2">
                               ...
                               <div className="flex items-center gap-4">
                                  ...
                                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
                                     ...
                                     {lang === 'tr' ? 'Canlı Haber Akışı' : 'Live News Feed'}
                                  </p>
                               </div>
                            </div>
                                  ...
                                  <div className="p-6 space-y-4 flex-1 flex flex-col">
                                     ...
                                     <div className="mt-auto pt-4 flex items-center justify-between border-t border-slate-700/50 text-[10px] font-bold text-slate-400">
                                        <span className="uppercase tracking-widest flex items-center gap-1.5 hover:text-white transition-colors">
                                           {lang === 'tr' ? 'Detayları Okuyun' : 'Read Details'}
                                        </span>
                       ...
                       <div className="space-y-12">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                             {(blogs.length > 0 ? blogs : [
                               { 
                                 title: lang === 'tr' ? 'Harika Bir Mülk Nasıl Değerlenir?' : 'How to Value a Great Property?', 
                                 summary: lang === 'tr' ? 'Doğru bir değerleme için sektörel tecrübe ve bölge hakimeyi gerekir...' : 'Industry experience is required for an accurate valuation...',
                                 date: '12 Eki 2026', 
                                 img: 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?q=80&w=800' 
                               },
                               { 
                                 title: lang === 'tr' ? 'Geleceğin Yatırım Noktaları' : 'Future Investment Spots', 
                                 summary: lang === 'tr' ? 'Yabancı yatırımcıların son dönemde yöneldiği trend lokasyonlar...' : 'Trending locations foreign investors are turning to lately...',
                                 date: '08 Eki 2026', 
                                 img: 'https://images.unsplash.com/photo-1570129477492-45c003edd2be?q=80&w=800' 
                               },
                               { 
                                 title: lang === 'tr' ? 'Ev Sahibi Olacakların Bilmesi Gerekenler' : 'Things to Know Before Buying', 
                                 summary: lang === 'tr' ? 'İlk evinizi alırken hukuksal süreçte eksik adım atmayın...' : 'Do not miss legal steps when buying your first home...',
                                 date: '28 Eyl 2026', 
                                 img: 'https://images.unsplash.com/photo-1560518883-ce09059eeffa?q=80&w=800' 
                               },
                             ]).map((blog, i) => (
                                <div key={i} className="group cursor-pointer flex flex-col bg-white border border-slate-200 rounded-3xl overflow-hidden hover:shadow-2xl hover:shadow-rose-500/10 transition-all duration-300">
                                   <div className="h-56 relative overflow-hidden">
                                      <div className="absolute inset-0 bg-cover bg-center transition-transform duration-700 group-hover:scale-105" style={{ backgroundImage: `url(${blog.image_url || blog.img || 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?q=80&w=800'})` }}></div>
                                   </div>
                                   <div className="p-6 space-y-4 flex-1 flex flex-col">
                                      <p className="text-[10px] font-black text-rose-500 uppercase tracking-widest">{blog.date || new Date(blog.created_at).toLocaleDateString()}</p>
                                      <h4 className="text-lg font-black text-slate-900 leading-snug group-hover:text-rose-600 transition-colors">
                                        {blog.title}
                                      </h4>
                                      <p className="text-xs text-slate-500 leading-relaxed max-w-sm line-clamp-2" dangerouslySetInnerHTML={{ __html: blog.summary || (blog.content ? blog.content.replace(/<[^>]*>?/gm, '').substring(0, 100) + '...' : '') }}></p>
                 ...
                 <div className="bg-slate-900 p-20 text-center relative overflow-hidden">
                    ...
                    <div className="relative z-10 space-y-8">
                       ...
                       <p className="text-slate-500 text-[10px] font-medium max-w-sm mx-auto uppercase tracking-widest leading-loose">
                         {lang === 'tr' ? 'LookPrice güvencesiyle gayrimenkul piyasasının zirvesine ulaşın.' : 'Reach the peak of real estate market powered by LookPrice.'}
                       </p>
