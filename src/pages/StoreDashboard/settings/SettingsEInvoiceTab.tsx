...
export const SettingsEInvoiceTab = ({
  ...
  return (
    ...
    >
      <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl p-6 md:p-8 rounded-2xl md:rounded-3xl border border-slate-200/80 dark:border-slate-800/80 shadow-xs space-y-8">
        <div className="flex items-center space-x-3 mb-2">
          <div className="p-3 bg-slate-100 dark:bg-slate-800 rounded-2xl text-slate-900 dark:text-white">
            <Building2 className="h-5 w-5" />
          </div>
          <div>
            <h3 className="text-lg font-bold text-slate-900 dark:text-white tracking-tight">{lang === 'tr' ? 'Resmi Belge & E-Fatura' : 'Official Docs & E-Invoice'}</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{lang === 'tr' ? 'E-Fatura sistemini açıp kapatın ve entegratör ayarlarınızı yapın.' : 'Enable/disable e-invoice system and configure your integrator.'}</p>
          </div>
        </div>

        {/* Toggle System */}
        <div className="space-y-4">
          <div className="flex items-center justify-between p-5 md:p-6 bg-slate-50/80 dark:bg-slate-800/40 rounded-2xl border border-slate-200/80 dark:border-slate-800/80">
            <div>
              <h4 className="font-bold text-slate-900 dark:text-slate-100 font-sans text-sm">{lang === 'tr' ? 'E-Fatura Sistemi Aktif' : 'E-Invoice System Active'}</h4>
              <p className="text-xs text-slate-500 dark:text-slate-400 font-medium leading-relaxed font-sans mt-0.5">{lang === 'tr' ? 'Eğer bu ülkede/mağazada e-fatura kullanmıyorsanız kapalı tutun.' : 'Keep this disabled if you do not use e-invoices in your country/store.'}</p>
            </div>
            <button
              type="button"
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors cursor-pointer ${branding?.einvoice_settings?.is_active ? 'bg-slate-900 dark:bg-white' : 'bg-slate-300 dark:bg-slate-700'}`}
              onClick={() => {
            ...
            >
              <span className={`inline-block h-4 w-4 transform rounded-full bg-white dark:bg-slate-900 transition-transform ${branding?.einvoice_settings?.is_active ? 'translate-x-[1.4rem]' : 'translate-x-[0.2rem]'}`} />
            </button>
          </div>

          <div className="flex items-center justify-between p-5 md:p-6 bg-slate-50/80 dark:bg-slate-800/40 rounded-2xl border border-slate-200/80 dark:border-slate-800/80">
            <div>
              <h4 className="font-bold text-slate-900 dark:text-slate-100 font-sans text-sm">{lang === 'tr' ? 'E-İrsaliye Sistemi Aktif' : 'E-Waybill System Active'}</h4>
              <p className="text-xs text-slate-500 dark:text-slate-400 font-medium leading-relaxed font-sans mt-0.5">{lang === 'tr' ? 'Eğer e-irsaliye (sevk irsaliyesi) kullanmıyorsanız bu seçeneği kapatın.' : 'Disable this if you do not use e-waybills (shipment waybills).'}</p>
            </div>
            <button
              type="button"
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors cursor-pointer ${branding?.einvoice_settings?.is_ewaybill_active ? 'bg-slate-900 dark:bg-white' : 'bg-slate-300 dark:bg-slate-700'}`}
              onClick={() => {
                const currentParams = branding?.einvoice_settings || { provider: 'none' };
                onBrandingChange('einvoice_settings', { ...currentParams, is_ewaybill_active: !currentParams.is_ewaybill_active });
              }}
            >
              <span className={`inline-block h-4 w-4 transform rounded-full bg-white dark:bg-slate-900 transition-transform ${branding?.einvoice_settings?.is_ewaybill_active ? 'translate-x-[1.4rem]' : 'translate-x-[0.2rem]'}`} />
            </button>
          </div>
        </div>
