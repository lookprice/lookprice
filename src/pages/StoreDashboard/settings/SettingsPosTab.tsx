...
import { CreditCard, Truck, Building2, Cpu, Hotel, Banknote, Receipt, QrCode, Scan, ExternalLink, Copy, Check, Printer } from "lucide-react";
        ...
        <div className="bg-white p-6 sm:p-8 rounded-[2.5rem] border border-slate-200/80 shadow-xs relative overflow-hidden">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-100">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-xl font-black text-slate-900 leading-tight tracking-tight font-sans">
                    {lang === 'tr' ? "Mağaza İçi 'Fiyat Gör' QR & Barkod Okuyucu" : "In-Store Price Check QR & Scanner"}
                  </h3>
                  <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase bg-amber-100 text-amber-800 border border-amber-200">
                    Kiosk & QR
                  </span>
                ...
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">
                  {lang === 'tr' ? 'Müşteri Kendi Cihazıyla Fiyat Öğrensin' : 'Self-Service Customer Price Check'}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="pt-6 grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
            {/* QR Visual Card */}
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 flex flex-col items-center text-center shadow-inner">
              <QRCodeSVG 
                value={scanUrl}
                size={140}
                level="H"
                includeMargin={true}
              />
              <span className="mt-2 text-[10px] font-mono font-bold text-slate-500 truncate max-w-[180px]">
                {scanUrl}
              </span>
            </div>
            ...
            <div className="md:col-span-2 space-y-4 text-left">
              <p className="text-xs text-slate-600 font-medium leading-relaxed">
                {lang === 'tr'
                  ? "Müşterileriniz mağazanızın reyonlarındaki veya kasadaki QR kodu telefon kamerasıyla okutarak doğrudan bu ekrana ulaşır. Ürünün barkodunu kameraya tuttuklarında güncel fiyatı, stok durumunu ve indirimleri anında görürler."
                  : "Customers scan this QR code with their mobile cameras in-store to open the Price Checker. Scanning any barcode displays real-time prices and stock."}
              </p>
              ...
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 flex items-center justify-between gap-2">
                ...
                >
                  ...
                  <span>{copiedScanUrl ? (lang === 'tr' ? 'Kopyalandı' : 'Copied') : (lang === 'tr' ? 'Kopyala' : 'Copy')}</span>
                </button>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2.5">
                ...
                >
                  ...
                  <span>{lang === 'tr' ? "Sayfayı Canlı Aç" : "Open Live Scanner"}</span>
                </a>
              </div>
            </div>
          </div>
