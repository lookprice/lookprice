...
const SettingsTab = ({ 
  ...
  return (
    <div className="space-y-6 md:space-y-8 max-w-6xl mx-auto pb-24">
      ...
      {activeSubTab === 'e-stores' && !isPortfolio && !isCafeRestaurant && (
        <SettingsEStoresTab
          branding={branding}
          onBrandingChange={onBrandingChange}
          lang={lang}
      ...
      {/* Unified Apple/Mercedes Minimalist Floating Sticky Save Bar */}
      <div className="fixed bottom-4 right-4 md:bottom-6 md:right-8 z-40 pointer-events-none">
        <div className="pointer-events-auto">
          <button 
            onClick={onSaveBranding}
            disabled={savingBranding}
            className={`flex items-center gap-3 px-5 py-3 text-white dark:text-slate-900 rounded-2xl md:rounded-full font-bold text-xs tracking-wider transition-all duration-300 shadow-2xl hover:scale-105 active:scale-95 border border-white/20 dark:border-slate-800/20 backdrop-blur-2xl ${savingBranding ? 'bg-slate-500/90 cursor-not-allowed' : 'bg-slate-900/95 dark:bg-white/95 hover:bg-slate-800 dark:hover:bg-slate-100 shadow-slate-900/25'}`}
          >
            <div className="p-1.5 bg-white/10 dark:bg-slate-900/10 rounded-full">
              {savingBranding ? (
            ...
            <span className="uppercase">
              {savingBranding 
                ...
                : (translations[lang]?.dashboard?.saveSettings || 'AYARLARI KAYDET')}
            </span>
          </button>
        </div>
      </div>
