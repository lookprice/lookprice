          ...
          {isSectionEnabled("portfolio") && (
            <div id="listings-section" className="space-y-12">
              <div className="flex flex-col md:flex-row md:items-end justify-between border-b-2 border-slate-100 pb-8 gap-4">
                ...
                {/* Satılık / Kiralık Ayraç Butonları */}
                <div className="flex gap-2 bg-slate-100 p-1.5 rounded-2xl border border-slate-205/60 shrink-0 self-start md:self-auto">
                  <button
                  ...
                  >
                    {lang === "tr" ? "TÜMÜ" : "ALL"} ({filteredProducts.length})
                  </button>
                  <button
                  ...
                  >
                    🏠 {lang === "tr" ? "SATILIK" : "FOR SALE"} ({filteredProducts.filter(p => p.status !== 'rented' && (p as any).status !== 'rented').length})
                  </button>
                  <button
                  ...
                  >
                    🔑 {lang === "tr" ? "KİRALIK" : "FOR RENT"} ({filteredProducts.filter(p => p.status === 'rented' || (p as any).status === 'rented').length})
                  </button>
                </div>
              </div>
                    ...
                    return (
                      ...
                      >
                        <div className="bg-slate-50 rounded-[3rem] overflow-hidden relative shadow-sm group-hover:shadow-2xl transition-all duration-700 aspect-[16/10]">
                          ...
                          ></div>
                          <div className="absolute top-8 left-8 flex flex-col sm:flex-row gap-2">
                            ...
                            {p.type === 'real_estate' && (
                              <div className={`backdrop-blur-md px-5 py-2.5 rounded-2xl text-[10px] font-black text-white uppercase tracking-[0.25em] shadow-xl flex items-center gap-1.5 ${
                                p.status === 'active' || !p.status ? 'bg-emerald-600/90' :
                                p.status === 'rented' ? 'bg-sky-600/90' :
                                p.status === 'optioned' ? 'bg-amber-600/90' :
                                'bg-rose-600/90'
                              }`}>
                                {p.status === 'active' || !p.status ? (lang === 'tr' ? 'SATILIK' : 'FOR SALE') :
                                 p.status === 'rented' ? (lang === 'tr' ? 'KİRALIK' : 'FOR RENT') :
                                 p.status === 'optioned' ? (lang === 'tr' ? 'OPSİYONLU' : 'OPTIONED') : 
