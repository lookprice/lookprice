...
export const RealEstateModal: React.FC<RealEstateModalProps> = ({ 
  ...
  const handleSave = () => {
    ...
    const sectorData = {
      ...
      zoning_status: (formData as any).zoning_status || ''
    };
  ...
  useEffect(() => {
    setValidationError(null);
    if (property) {
      const sec = typeof (property as any).sector_data === 'string' ? (() => { try { return JSON.parse((property as any).sector_data); } catch(e) { return {}; } })() : ((property as any).sector_data || {});
      setFormData({
        ...sec,
        ...property,
        ...
        ada: (property as any).ada || sec.ada || (property.block_plot ? property.block_plot.split('/')[0] : ''),
        parsel: (property as any).parsel || sec.parsel || (property.block_plot ? property.block_plot.split('/')[1] : ''),
        mahalle: (property as any).mahalle || sec.mahalle || '',
        kocan_type: (property as any).kocan_type || sec.kocan_type || property.kktc_title_type || '',
        zoning_status: (property as any).zoning_status || sec.zoning_status || '',
