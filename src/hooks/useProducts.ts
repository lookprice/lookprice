...
export const useProducts = (user: any, slug: string | undefined, includeBranches: boolean, branding: any, planLimits: Record<string, number>, lang: string) => {
  ...
  const { handleAddProduct, handleDeleteProduct, handleDeleteAllProducts, editingProduct, setEditingProduct } = useProductActions(user, currentStoreId, products, lang, fetchData);
