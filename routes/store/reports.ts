...
router.get("/analytics", async (req: any, res) => {
  ...
  try {
    ...
    let horecaStats: any = {
      ...
      cancelled_sales_amount: 0,
      complimentary_items_count: 0,
      complimentary_items_amount: 0,
      total_tables: 0,
      occupied_tables: 0,
    };
    ...
    try {
      ...
      `, [storeId, currentMonthStart, currentMonthEnd]);
      horecaStats.hourly_sales = hourlyRes.rows;
      ...
      const paymentRes = await pool.query(`
        ...
        ORDER BY amount DESC
      `, [storeId, currentMonthStart, currentMonthEnd]);
      horecaStats.payment_methods = paymentRes.rows;

      const serviceRes = await pool.query(`
        SELECT 
          CASE 
            WHEN customer_name ILIKE '%Oda%' THEN 'room'
            WHEN customer_name ILIKE '%Paket%' OR customer_name ILIKE '%Gel-Al%' THEN 'takeaway'
            ELSE 'dine_in'
          END as service_type,
          SUM(total_amount)::FLOAT as amount,
          COUNT(id)::INT as count
        ...
        GROUP BY 1
      `, [storeId, currentMonthStart, currentMonthEnd]);
      horecaStats.service_types = serviceRes.rows;
      ...
      const menuRes = await pool.query(`
        ...
        LIMIT 10
      `, [storeId, currentMonthStart, currentMonthEnd]);
      horecaStats.top_menu_items = menuRes.rows;

      const adisyonRes = await pool.query(`
        SELECT 
          COUNT(id)::INT as total_adisyon,
          COALESCE(AVG(total_amount), 0)::FLOAT as avg_adisyon
        FROM sales
        WHERE store_id = $1 AND status = 'completed' AND created_at BETWEEN $2 AND $3
      `, [storeId, currentMonthStart, currentMonthEnd]);
      if (adisyonRes.rows.length > 0) {
        horecaStats.total_adisyon = adisyonRes.rows[0].total_adisyon || 0;
        horecaStats.avg_adisyon = adisyonRes.rows[0].avg_adisyon || 0;
      }
      ...
      const cancelledRes = await pool.query(`
        ...
        FROM sales
        WHERE store_id = $1 AND status = 'cancelled' AND created_at BETWEEN $2 AND $3
      `, [storeId, currentMonthStart, currentMonthEnd]);
      if (cancelledRes.rows.length > 0) {
        horecaStats.cancelled_sales_count = cancelledRes.rows[0].count || 0;
        horecaStats.cancelled_sales_amount = cancelledRes.rows[0].amount || 0;
      }

      const compRes = await pool.query(`
        SELECT 
          COUNT(si.id)::INT as count,
          COALESCE(SUM(si.quantity * COALESCE(p.price, 0)), 0)::FLOAT as amount
        FROM sale_items si
        JOIN sales s ON si.sale_id = s.id
        LEFT JOIN products p ON si.product_id = p.id
        WHERE s.store_id = $1 AND s.created_at BETWEEN $2 AND $3
          AND (si.unit_price = 0 OR s.notes ILIKE '%ikram%' OR si.product_name ILIKE '%ikram%')
      `, [storeId, currentMonthStart, currentMonthEnd]);
      if (compRes.rows.length > 0) {
        horecaStats.complimentary_items_count = compRes.rows[0].count || 0;
        horecaStats.complimentary_items_amount = compRes.rows[0].amount || 0;
      }

      const tablesRes = await pool.query(`
        SELECT 
          COUNT(*)::INT as total_tables,
          COUNT(CASE WHEN status = 'occupied' THEN 1 END)::INT as occupied_tables
        FROM restaurant_tables
        WHERE store_id = $1
      `, [storeId]);
      if (tablesRes.rows.length > 0) {
        horecaStats.total_tables = tablesRes.rows[0].total_tables || 0;
        horecaStats.occupied_tables = tablesRes.rows[0].occupied_tables || 0;
      }
    } catch (horecaErr) {
      console.error("Horeca Analytics Error:", horecaErr);
    }
