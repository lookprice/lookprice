...
router.post("/", async (req: any, res) => {
  try {
    ...
    const store_name = cleanedBody.store_name || cleanedBody.name || existingStore.name;
    const name = store_name;
    const contact_person = cleanedBody.contact_person !== undefined ? cleanedBody.contact_person : existingStore.contact_person;
    ...
    const country = cleanedBody.country !== undefined ? cleanedBody.country : (existingStore.country || 'TR');
    const default_currency = cleanedBody.default_currency !== undefined ? cleanedBody.default_currency : (existingStore.default_currency || 'TRY');
    const language = cleanedBody.language !== undefined ? cleanedBody.language : (existingStore.language || 'tr');
    
    // Hotel Concept & License Protection (Kara Kaplı Kitap & Anti-Regression Rule)
    // The store-level hotel license (stores.hotel_module_enabled) is controlled solely by SuperAdmin.
    // Store-level branding updates MUST NEVER strip or downgrade an existing hotel license.
    const hasHotelLicense = Boolean(
      existingStore.hotel_module_enabled || 
      existingBranding.hotel_license_enabled || 
      existingBranding.hotel_module_enabled
    );
    
    let storeHotelLicense = hasHotelLicense;
    if (req.user.role === 'superadmin' && cleanedBody.hotel_module_enabled !== undefined) {
      storeHotelLicense = Boolean(cleanedBody.hotel_module_enabled);
    }

    // Module toggle inside store: if store has license, operator can toggle feature or keep current
    let requestedHotel = storeHotelLicense;
    if (storeHotelLicense) {
      if (cleanedBody.hotel_module_enabled !== undefined) {
        requestedHotel = Boolean(cleanedBody.hotel_module_enabled);
      } else if (cleanedBody.branding && cleanedBody.branding.hotel_module_enabled !== undefined) {
        requestedHotel = Boolean(cleanedBody.branding.hotel_module_enabled);
      } else if (existingBranding.hotel_module_enabled !== undefined) {
        requestedHotel = Boolean(existingBranding.hotel_module_enabled);
      }
    } else {
      requestedHotel = false;
    }
    const hotel_module_enabled = requestedHotel;
    ...
    // Sync all resolved values onto updatedBranding
    updatedBranding.store_name = name;
    updatedBranding.name = name;
    updatedBranding.contact_person = contact_person || '';
    updatedBranding.phone = phone || '';
    updatedBranding.address = address || '';
    updatedBranding.email = email || '';
    updatedBranding.country = country;
    updatedBranding.default_currency = default_currency;
    updatedBranding.language = language;
    updatedBranding.store_type = store_type;
    updatedBranding.sub_sector = sub_sector;
    updatedBranding.hotel_license_enabled = storeHotelLicense;
    updatedBranding.hotel_module_enabled = hotel_module_enabled;
...
router.post("/sync-tcmb", async (req: any, res) => {
  try {
    ...
    syncMarketplacePricesOnRateChange(targetStoreId, newRates).catch((syncErr: any) => {
      console.warn(`[TCMB-SYNC-ROUTE] Store #${targetStoreId} pazaryeri fiyat revizyonu hatası:`, syncErr.message || syncErr);
    });
