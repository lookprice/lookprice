...
router.get("/stores", async (req: any, res) => {
  ...
  const normalized = stores.rows.map(row => {
    let br = row.branding;
    if (typeof br === 'string') {
    ...
    const isHotel = Boolean(row.hotel_module_enabled || br.hotel_module_enabled || br.hotel_license_enabled);
    return {
      ...row,
...
// SuperAdmin: 1-click Toggle Hotel Concept
router.post("/stores/:id/toggle-hotel", async (req: any, res) => {
  const storeId = req.params.id;
  const { enabled } = req.body;
  try {
    const existing = await pool.query("SELECT hotel_module_enabled, branding FROM stores WHERE id = $1", [storeId]);
    ...
    const currentHotel = Boolean(existing.rows[0].hotel_module_enabled || br.hotel_module_enabled || br.hotel_license_enabled);
    const newStatus = enabled !== undefined ? Boolean(enabled) : !currentHotel;

    br.hotel_module_enabled = newStatus;
    br.hotel_license_enabled = newStatus;

    await pool.query(`
      UPDATE stores 
      SET hotel_module_enabled = $1, branding = $2 
      WHERE id = $3
    `, [newStatus, JSON.stringify(br), storeId]);

    res.json({ success: true, hotel_module_enabled: newStatus });
  } catch (e: any) {
    res.status(500).json({ error: e.message });
  }
});
...
router.post("/stores", async (req: any, res) => {
  const { 
    ...
    admin_email, admin_password, default_currency, language, plan, parent_id, store_type, sub_sector, hotel_module_enabled,
    status, is_approved, max_products, max_properties, max_vehicles, max_users, max_customers 
  } = req.body;
  try {
    await pool.query("BEGIN");
    // Initialize branding based on store_type & store info
    const defaultBranding = {
      ...
      hotel_module_enabled: !!hotel_module_enabled,
      hotel_license_enabled: !!hotel_module_enabled,
    ...
    const storeRes = await pool.query(
      `INSERT INTO stores (
        name, slug, address, contact_person, phone, country, email, subscription_end, 
        default_currency, language, plan, parent_id, store_type, sub_sector, hotel_module_enabled,
      ...
      [
        ...
        default_currency || 'TRY', language || 'tr', plan || 'free', parent_id || null, store_type || 'product', sub_sector || null, !!hotel_module_enabled,
...
router.put("/stores/:id", async (req: any, res) => {
  const { 
    name, slug, address, contact_person, phone, country, email, subscription_end, 
    default_currency, language, admin_password, plan, parent_id, store_type, sub_sector, hotel_module_enabled,
    status, is_approved, max_products, max_properties, max_vehicles, max_users, max_customers 
  } = req.body;
  try {
    await pool.query("BEGIN");

    // Fetch existing store to update branding object
    ...
    // Always keep store details in branding JSON synchronized with store record
    currentBranding.store_name = name;
    currentBranding.name = name;
    currentBranding.contact_person = contact_person || '';
    currentBranding.phone = phone || '';
    currentBranding.address = address || '';
    currentBranding.email = email || '';
    currentBranding.country = country || 'TR';
    currentBranding.default_currency = default_currency || 'TRY';
    currentBranding.language = language || 'tr';
    currentBranding.store_type = store_type || 'product';
    currentBranding.sub_sector = sub_sector || null;

    const isHotelRequested = hotel_module_enabled !== undefined 
      ? Boolean(hotel_module_enabled) 
      : Boolean(existingStore.rows[0]?.hotel_module_enabled || currentBranding.hotel_module_enabled || currentBranding.hotel_license_enabled);

    currentBranding.hotel_module_enabled = isHotelRequested;
    currentBranding.hotel_license_enabled = isHotelRequested;
    if (!currentBranding.page_layout_settings) currentBranding.page_layout_settings = {};
    currentBranding.page_layout_settings.sector = store_type || 'product';

    await pool.query(`
      UPDATE stores 
      SET name = $1, slug = $2, address = $3, contact_person = $4, phone = $5, country = $6, email = $7, subscription_end = $8, 
          default_currency = $9, language = $10, plan = $11, parent_id = $12, store_type = $13, sub_sector = $14, hotel_module_enabled = $15,
          status = $16, is_approved = $17, max_products = $18, max_properties = $19, max_vehicles = $20, max_users = $21, max_customers = $22,
          branding = $23
      WHERE id = $24
    `, [
      name, slug, address, contact_person, phone, country || 'TR', email, subscription_end, 
      default_currency || 'TRY', language || 'tr', plan || 'free', parent_id || null, store_type || 'product', sub_sector || null, isHotelRequested,
      status !== undefined ? status : 'approved',
      is_approved !== undefined ? is_approved : true,
      max_products !== undefined ? Number(max_products) : 100,
      max_properties !== undefined ? Number(max_properties) : 20,
