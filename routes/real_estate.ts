...
(async () => {
  try {
    await pool.query(`
      ...
      DO $$ BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='real_estate_properties' AND column_name='listing_features') THEN
          ALTER TABLE real_estate_properties ADD COLUMN listing_features TEXT[];
        END IF;
      END $$;
    `);
