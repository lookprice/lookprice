...
router.post("/ai-3d-tour", async (req: any, res) => {
  const { name, type, files } = req.body;
  const apiKey = getGeminiApiKey();

  try {
    const isRealEstate = type === 'real_estate';
    // Generate a beautiful navigation tour object simulation using Gemini
    let prompt = "";
    if (isRealEstate) {
...
router.post("/ai-virtual-staging", async (req: any, res) => {
...
router.post("/ai-image-enhance", async (req: any, res) => {
...
router.post("/ai-blur-privacy", async (req: any, res) => {
